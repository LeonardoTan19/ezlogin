...
fn app_config_dir() -> Result<PathBuf, String> {
    ...
    let dir = dirs::config_dir()
        ...
        .ok_or_else(|| "无法定位配置目录：缺少 HOME/XDG_CONFIG_HOME".to_string())?;

    Ok(dir.join("ezlogin"))
}

fn login_options_path() -> Result<PathBuf, String> {
    Ok(app_config_dir()?.join("login-options.json"))
}
