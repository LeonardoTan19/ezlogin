...
impl PortalClient {
    ...
    pub async fn init_session(&mut self) -> Result<()> {
        ...
        #[cfg(target_os = "android")]
        {
            for path in static_paths {
                self.request(Method::GET, path, Some(&auth_url), empty.clone(), None)
                    .await?;
            }
            self.request(Method::GET, &valid_code, Some(&auth_url), empty.clone(), None)
                .await?;
        }
        ...
        #[cfg(not(target_os = "android"))]
        {
            let (a, b, c, d) = tokio::join!(
                self.static_get(static_paths[0], &auth_url),
                self.static_get(static_paths[1], &auth_url),
                self.static_get(static_paths[2], &auth_url),
                self.static_get(&valid_code, &auth_url),
            );
            a?;
            b?;
            c?;
    ...
    #[cfg(not(target_os = "android"))]
    async fn static_get(&self, path_or_url: &str, referer: &str) -> Result<()> {
        let url = if path_or_url.starts_with("http") {
            path_or_url.to_string()
        } else {
            format!("{}{}", self.base_url, path_or_url)
        ...
        self.client
            ...
            .send()
            .await
            .with_context(|| format!("static GET failed: {}", summarize_url(&url)))?;
        Ok(())
    }
    ...
    pub async fn probe_connectivity(&self, retries: u32, interval: Duration) -> Result<bool> {
        ...
        for attempt in 1..=retries {
            for url in probes {
                let response = self.probe_client.get(url).timeout(self.timeout).send().await;
...
fn current_time_for_valid_code() -> String {
    let beijing = chrono::FixedOffset::east_opt(8 * 3600).expect("valid +0800 offset");
    chrono::Utc::now()
