...
use ndarray::{Array, Array4, ArrayViewD, Axis};
...
fn decode_ctc(logits: &ArrayViewD<f32>, dict: &[String]) -> Result<(String, f32)> {
...
impl OcrEngine {
    ...
    pub fn recognize(&mut self, image_bytes: &[u8]) -> Result<OcrResult> {
        ...
        let (text, confidence) = decode_ctc(&logits.view(), &self.dict)?;
