...
function App() {
  ...
  useEffect(() => {
    const loadSaved = async () => {
      const [savedResult, optionsResult, mobileResult] = await Promise.allSettled([
      ...
      if (mobileResult.status === "fulfilled") {
        setIsMobilePlatform(Boolean(mobileResult.value));
      }

      if (savedResult.status === "fulfilled" && savedResult.value) {
        setAccount(savedResult.value.account ?? "");
        setPassword(savedResult.value.password ?? "");
      }
  ...
  const isLinuxDesktop =
    !isMobilePlatform && navigator.userAgent.toLowerCase().includes("linux");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`登录成功，尝试次数：${result.attempt}`);
    } catch (e) {
      setError(toErrorText(e, "登录请求失败"));
      setErrorTitle("登录失败");
      setFailureKind(null);
    } finally {
      setIsLoading(false);
  ...
  async function handleOpenNetworkSettings() {
    ...
    try {
      ...
      setError(toErrorText(e, "打开网络设置失败"));
  ...
  async function handleSaveConfig() {
    ...
    try {
      ...
      await Promise.all([
        invoke("save_credentials", { account: account.trim(), password }),
        invoke("save_login_options", {
          options: {
